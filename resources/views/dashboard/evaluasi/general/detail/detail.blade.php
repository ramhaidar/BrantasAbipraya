                ...
                <div class="ibox">
                    <div class="ibox-head pe-0 ps-0">
                        <div class="ibox-title d-flex align-items-center gap-3">
                            <a class="btn btn-outline-primary btn-sm" href="{{ route('evaluasi_rkb_general.index') }}">
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            <a class="btn btn-warning btn-sm me-2 {{ $rkb->is_approved_svp ? '' : 'disabled' }}" href="{{ route('export.evaluasi_rkb_general', ['id' => $rkb->id]) }}">
                            ...
                            <!-- Tombol Approve VP -->
                            <button class="btn btn-primary btn-sm" id="approveVpButton" data-bs-toggle="modal" data-bs-target="#modalForApproveVp" data-action="{{ route('evaluasi_rkb_general.detail.approve.vp', $rkb->id) }}" data-message="Apakah Anda yakin ingin Approve RKB ini sebagai VP?" {{ !$rkb->is_evaluated ? 'disabled' : '' }} {{ $rkb->is_approved_vp ? 'disabled' : '' }}>
                                <i class="fa fa-check"></i> <span class="ms-2">Approve RKB (VP)</span>
                            </button>

                            <!-- Tombol Approve SVP -->
                            <button class="btn btn-primary btn-sm" id="approveSvpButton" data-bs-toggle="modal" data-bs-target="#modalForApproveSvp" data-action="{{ route('evaluasi_rkb_general.detail.approve.svp', $rkb->id) }}" data-message="Apakah Anda yakin ingin Approve RKB ini sebagai SVP?" {{ !$rkb->is_approved_vp ? 'disabled' : '' }} {{ $rkb->is_approved_svp ? 'disabled' : '' }}>
