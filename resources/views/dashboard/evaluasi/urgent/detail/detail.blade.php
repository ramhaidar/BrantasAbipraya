                    ...
                    <div class="ibox-head pe-0 ps-0">
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            @if (!$rkb->is_approved_vp && !$rkb->is_approved_svp)
                                @if ($rkb->is_evaluated)
                                    <button class="btn btn-danger btn-sm" id="evaluateBtnButton" data-bs-toggle="modal" data-bs-target="#modalForEvaluate" data-action="{{ route('evaluasi_rkb_urgent.detail.evaluate', $rkb->id) }}" data-message="Apakah Anda yakin ingin membatalkan hasil Evaluasi RKB ini?">
                            ...
                            <!-- Tombol Approve VP -->
                            <button class="btn btn-primary btn-sm" id="approveVpButton" data-bs-toggle="modal" data-bs-target="#modalForApproveVp" data-action="{{ route('evaluasi_rkb_urgent.detail.approve.vp', $rkb->id) }}" data-message="Apakah Anda yakin ingin Approve RKB ini sebagai VP?" {{ !$rkb->is_evaluated ? 'disabled' : '' }} {{ $rkb->is_approved_vp ? 'disabled' : '' }}>
                                <i class="fa fa-check"></i> <span class="ms-2">Approve RKB (VP)</span>
                            </button>

                            <!-- Tombol Approve SVP -->
                            <button class="btn btn-primary btn-sm" id="approveSvpButton" data-bs-toggle="modal" data-bs-target="#modalForApproveSvp" data-action="{{ route('evaluasi_rkb_urgent.detail.approve.svp', $rkb->id) }}" data-message="Apakah Anda yakin ingin Approve RKB ini sebagai SVP?" {{ !$rkb->is_approved_vp ? 'disabled' : '' }} {{ $rkb->is_approved_svp ? 'disabled' : '' }}>
