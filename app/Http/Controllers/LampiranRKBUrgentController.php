...
class LampiranRKBUrgentController extends Controller
{
    public function store ( Request $request )
    {
        ...
        try
        {
            ...
            $linkAlatDetailRKB = LinkAlatDetailRKB::findOrFail ( $validatedData[ 'id_link_alat_detail_rkb' ] );
            ...
            if ( $request->hasFile ( 'lampiran' ) )
            {
                ...
                // Tentukan folder penyimpanan berdasarkan nomor RKB
    ...
    {
        $lampiran          = LampiranRKBUrgent::with ( 'linkAlatDetailRKB' )->find ( $id );
        $linkAlatDetailRKB = LinkAlatDetailRKB::find ( $lampiran->linkAlatDetailRKB->id );

        // remove the file and folder from storage
        $filePath   = public_path ( 'storage/' . $lampiran->file_path );
